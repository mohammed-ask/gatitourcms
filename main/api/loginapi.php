...
if (isset($_GET['mobile']) && isset($_GET['password'])) {
    ...
    // Query the database
    $data = $obj->selectextrawhereupdate("users", "type,id,name,email,mobile,whatsappno", "(`mobile` = '" . $mobile . "' ) and password = '" . $password . "' and status != 99")->fetch_assoc();
    ...
    echo json_encode(['error' => 'Mobile/Password is required']);
}
// run SQL statement
