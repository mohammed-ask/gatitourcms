...
if ($obj->checktoken()) {
    ...
    $tbook = $obj->selectextrawhereupdate("ticketbooking", "name,`from`,`to`,DATE_FORMAT(pickupat, '%m/%d/%Y %h:%i %p') AS pickupat,mobileno,traveltype", "vehicleid in (" . $vids . ") and status =1 and date(pickupat) >= date(CONVERT_TZ(NOW(),'+00:00','$timeskip'))");
