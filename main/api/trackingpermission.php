...
if ($obj->checktoken()) {
    if (isset($_GET['trackenable'])) {

        $xx["trackenable"] = $_GET['trackenable'];
        // Query the database
        $dev = $obj->updatewhere("users", $xx, "apptoken = '" . getAuthorizationHeader() . "'");
        ...
        echo json_encode(['error' => 'Tracking Permission is required']);
    }
} else {
    header('HTTP/1.1 400 Bad Request');
