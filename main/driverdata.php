...
while ($row = $obj->fetch_assoc($result)) {
    ...
    $n[] = "<a target='_blank' href='https://www.google.com/search?q=" . round($row['lat'], 4) . "+" . round($row['long'], 4) . "'> Latitude : " . round($row['lat'], 4) . " Longitude : " . round($row['long'], 4) . " on " . changedateformatespecito($row['lastupdate'], "Y-m-d H:i:s", "d/m/Y h:i a") . "<a>";
    $a = "";
