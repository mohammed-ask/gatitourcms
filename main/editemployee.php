...
<form style="overflow-x: hidden;" id="adduser" onsubmit="event.preventDefault();sendForm('id', '<?= $id ?>', 'updatedriver', 'resultid', 'adduser');return 0;">
    ...
    <div class="row">
        <label class="col-6 block text-sm" style="margin-bottom: 5px;">
            ...
            <input data-bvalidator="required,digit,minlength[10],maxlength[10]" name="mobile" value="<?= $rowuser['mobile'] ?>" class="form-control" placeholder="Employee's Mobile No." />
        </label>
        <label class="col-6 block text-sm" style="margin-bottom: 5px;">
            <span class="text-gray-700 dark:text-gray-400">Password</span>
            <input type="password" data-bvalidator="required,maxlength[8],minlength[4]" id="password" value="<?= $rowuser['password'] ?>" name="password" class="form-control" placeholder="Enter Password!" />
        ...
        <label class="col-6 block text-sm" style="margin-bottom: 5px;">
            ...
            <select data-bvalidator="required" class="form-control select2" name="role" id="role">
                ...
                foreach ($emprole as list($id, $name)) { ?>
                    <option value="<?php echo $id; ?>" <?= ($id == $rowuser['role']) ? 'selected' : '' ?>> <?php echo $name; ?></option>
                <?php
                } ?>
            </select>
        </label>
